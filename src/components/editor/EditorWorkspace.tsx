...
export function EditorWorkspace() {
  ...
  return <div className="flex h-full min-h-0 flex-col bg-white text-foreground">
    ...
    <div className="flex min-h-0 flex-1 overflow-hidden bg-white">
      ...
      <EditorChrome nodes={nodes} rootIds={topLevel} selected={selected} onSelect={(id) => editorStore.select([id], id)} onToggleLock={(id) => editorStore.toggleLock(id)} />
...
function CanvasNode({ id, nodes, selected, onSelect }: { id: NodeId; nodes: Record<NodeId, EditorNode>; selected: NodeId | null; onSelect: (id: NodeId) => void }) {
  ...
  return <div className={`${isShape ? "absolute" : "relative mb-4"} rounded-md border p-5 transition-shadow ${isSelected ? "border-primary ring-2 ring-primary/20" : "border-transparent hover:border-primary/30"} ${node.locked ? "select-none" : ""}`} style={node.styles as CSSProperties} onClick={(event) => { event.stopPropagation(); onSelect(id); }}>
