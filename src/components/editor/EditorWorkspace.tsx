    ...
    <div className="flex h-full min-h-0 flex-col bg-white text-foreground">
      <div className="flex h-11 shrink-0 items-center justify-between border-b border-border bg-white px-2">
        <div className="flex items-center gap-1">
          ...
          <span className="px-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Design</span>
        </div>
        <div className="flex items-center gap-1 rounded-lg border border-border bg-white p-1">
          {(Object.keys(breakpointIcons) as EditorBreakpoint["id"][]).map((breakpoint) => {
            const Icon = breakpointIcons[breakpoint];
            return <button key={breakpoint} title={breakpoint} onClick={() => setBreakpoint(breakpoint)} className={`rounded-md p-1.5 ${state.viewport.breakpoint === breakpoint ? "bg-muted text-primary shadow-sm" : "text-muted-foreground hover:text-foreground"}`}><Icon className="size-4" /></button>;
          })}
        </div>
        <div className="flex items-center gap-1">
          ...
          {selectedNode && <button title={selectedNode.locked ? "Unlock selected" : "Lock selected"} onClick={() => editorStore.toggleLock(selectedNode.id)} className="ml-2 rounded-md border border-border p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground"><Lock className="size-3.5" /></button>}
          <button className="ml-2 inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-xs font-medium hover:bg-muted"><Eye className="size-3.5" /> Preview</button>
        </div>
      </div>

      <div className="flex min-h-0 flex-1 overflow-hidden bg-white">
        <aside className="flex w-12 shrink-0 flex-col items-center gap-1 border-r border-border bg-white py-2">
          <ToolButton active={tool === "select"} label="Select" onClick={() => setTool("select")}><MousePointer2 className="size-4" /></ToolButton>
          <ToolButton active={tool === "shape"} label="Draw rectangle" onClick={() => setTool("shape")}><Square className="size-4" /></ToolButton>
          <div className="my-1 h-px w-7 bg-border" />
          <ToolButton label="Add section" onClick={() => addNode("section")}><Plus className="size-4" /></ToolButton>
        </aside>

        <div className="min-h-0 flex-1 overflow-auto bg-white p-8">
          <div className="mx-auto min-h-[720px] origin-top bg-white shadow-xl transition-[width] duration-200" style={{ width: state.viewport.breakpoint === "desktop" ? "1200px" : state.viewport.breakpoint === "tablet" ? "768px" : "390px", transform: `scale(${state.viewport.zoom})`, transformOrigin: "top center" }}>
            <div
              className={`relative min-h-[720px] p-8 ${tool === "shape" ? "cursor-crosshair" : ""}`}
              onPointerDown={drawShape}
              onPointerUp={finishShape}
            >
              {topLevel.length === 0 ? <EmptyCanvas onAdd={() => addNode("section")} /> : topLevel.map((id) => <CanvasNode key={id} id={id} nodes={nodes} selected={selected} onSelect={(nodeId) => { if (tool === "select") editorStore.select([nodeId], nodeId); }} />)}
            </div>
          </div>
        </div>
      </div>
...
function CanvasNode({ id, nodes, selected, onSelect }: { id: NodeId; nodes: Record<NodeId, EditorNode>; selected: NodeId | null; onSelect: (id: NodeId) => void }) {
  ...
  return (
    <div
      className={`${isShape ? "absolute" : "relative mb-4"} ${isShape ? "cursor-pointer" : ""} rounded-md border p-5 transition-shadow ${isSelected ? "border-primary ring-2 ring-primary/20" : "border-transparent hover:border-primary/30"} ${node.locked ? "select-none" : ""}`}
...
function EmptyCanvas({ onAdd }: { onAdd: () => void }) {
  return <div className="flex min-h-[620px] items-center justify-center rounded border border-dashed border-border bg-muted/10"><div className="text-center"><p className="text-sm font-medium text-foreground">Your canvas is ready</p><p className="mt-1 text-xs text-muted-foreground">Choose a tool on the left or start with a section.</p><button onClick={onAdd} className="mt-4 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground">Add your first section</button></div></div>;
}

function ToolbarButton({ label, disabled, onClick, children }: { label: string; disabled?: boolean; onClick: () => void; children: ReactNode }) {
  return <button title={label} disabled={disabled} onClick={onClick} className="rounded-md p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground disabled:cursor-not-allowed disabled:opacity-40">{children}</button>;
}

function ToolButton({ label, active, onClick, children }: { label: string; active?: boolean; onClick: () => void; children: ReactNode }) {
  return <button title={label} aria-label={label} onClick={onClick} className={`rounded-md p-2 ${active ? "bg-muted text-primary" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}>{children}</button>;
}
