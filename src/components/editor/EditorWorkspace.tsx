...
export function EditorWorkspace() {
  ...
  const addNode = (type: "section" | "heading" | "text" | "button") => {
    const parentId =
      selected && nodes[selected] && !nodes[selected].locked ? selected : state.document.rootNodeId;
    ...
    <div className="flex h-full min-h-0 flex-col bg-white text-foreground">
      <div className="flex h-11 shrink-0 items-center justify-between border-b border-border bg-white px-2">
          ...
          {(Object.keys(breakpointIcons) as EditorBreakpoint["id"][]).map((breakpoint) => {
            ...
            return (
              <button
                ...
                className={`rounded-md p-1.5 ${state.viewport.breakpoint === breakpoint ? "bg-muted text-primary shadow-sm" : "text-muted-foreground hover:text-foreground"}`}
              >
        ...
        <div className="flex items-center gap-1">
          <ToolbarButton
            label="Zoom out"
            onClick={() =>
              editorStore.setViewport({ zoom: Math.max(0.25, state.viewport.zoom - 0.1) })
            }
          >
          ...
          <ToolbarButton
            label="Zoom in"
            onClick={() =>
              editorStore.setViewport({ zoom: Math.min(2, state.viewport.zoom + 0.1) })
            }
          >
          ...
          {selectedNode && (
            <button
              title={selectedNode.locked ? "Unlock selected" : "Lock selected"}
              onClick={() => editorStore.toggleLock(selectedNode.id)}
              className="ml-2 rounded-md border border-border p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground"
            >
              <Lock className="size-3.5" />
            </button>
          )}
          <button className="ml-2 inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-xs font-medium hover:bg-muted">
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden bg-white">
        <aside className="flex w-12 shrink-0 flex-col items-center gap-1 border-r border-border bg-white py-2">
          ...
          <ToolButton label="Add section" onClick={() => addNode("section")}>
        ...
        <EditorChrome
          ...
          rootIds={topLevel}
          selected={selected}
          onSelect={(id) => editorStore.select([id], id)}
          onToggleLock={(id) => editorStore.toggleLock(id)}
        />
              ...
              {topLevel.length === 0 ? (
                ...
                topLevel.map((id) => (
                  <CanvasNode
                    ...
                    selected={selected}
                    onSelect={(nodeId) => {
                      if (tool === "select") editorStore.select([nodeId], nodeId);
                    }}
                  />
                ))
              )}
      ...
      {tool === "shape" && (
        <div className="absolute bottom-4 left-1/2 z-10 -translate-x-1/2 rounded-full border border-border bg-white px-4 py-2 text-xs font-medium shadow-lg">
          Drag on the canvas to draw a rectangle · it will lock after creation
        </div>
      )}
    </div>
  );
...
function CanvasNode({
  ...
  return (
    <div
      className={`${isShape ? "absolute" : "relative mb-4"} rounded-md border p-5 transition-shadow ${isSelected ? "border-primary ring-2 ring-primary/20" : "border-transparent hover:border-primary/30"} ${node.locked ? "select-none" : ""}`}
    ...
    >
      ...
      {isShape ? null : node.type === "section" || node.type === "container" ? (
    ...
    <div className="flex min-h-[620px] items-center justify-center rounded border border-dashed border-border bg-muted/10">
      <div className="text-center">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Choose a tool on the left or start with a section.
        </p>
        <button
          onClick={onAdd}
...
function ToolButton({
  ...
  return (
    <button
      ...
      aria-label={label}
      onClick={onClick}
      className={`rounded-md p-2 ${active ? "bg-muted text-primary" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}
    >
      {children}
    </button>
  );
