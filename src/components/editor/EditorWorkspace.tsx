...
export function EditorWorkspace() {
  ...
  const addNode = (type: "section" | "heading" | "text" | "button") => {
    ...
    const withChild = appendChild({ ...nodes, [node.id]: node }, parentId, node.id);
  ...
  const drawShape = (event: PointerEvent<HTMLDivElement>) => {
    ...
    drawStart.current = { x: Math.max(0, (event.clientX - rect.left) / state.viewport.zoom), y: Math.max(0, (event.clientY - rect.top) / state.viewport.zoom) };
  ...
  const finishShape = (event: PointerEvent<HTMLDivElement>) => {
    ...
    const rect = event.currentTarget.getBoundingClientRect();
    const endX = Math.max(0, (event.clientX - rect.left) / state.viewport.zoom); const endY = Math.max(0, (event.clientY - rect.top) / state.viewport.zoom); const start = drawStart.current;
    const width = Math.max(24, Math.abs(endX - start.x)); const height = Math.max(24, Math.abs(endY - start.y)); const left = Math.min(start.x, endX); const top = Math.min(start.y, endY);
    const node = createEditorNode("shape", { parentId: state.document.rootNodeId, locked: true, props: { shape: "rectangle" }, styles: { position: "absolute", left: `${left}px`, top: `${top}px`, width: `${width}px`, height: `${height}px`, background: "#e5e7eb", borderRadius: "8px", border: "1px solid #cbd5e1" } });
    const withChild = appendChild({ ...state.document.nodes, [node.id]: node }, state.document.rootNodeId, node.id);
    editorStore.commit({ ...state.document, nodes: withChild }, "Draw rectangle"); editorStore.select([node.id], node.id); drawStart.current = null; drawing.current = false; setTool("select");
  };
  const selectedNode = selected ? nodes[selected] : undefined;
  return <div className="flex h-full min-h-0 flex-col bg-white text-foreground">
    ...
    <div className="flex min-h-0 flex-1 overflow-hidden bg-white">
      ...
      <EditorChrome nodes={nodes} rootIds={topLevel} selected={selected} onSelect={(id) => editorStore.select([id], id)} onToggleLock={(id) => editorStore.toggleLock(id)} onUpdateStyle={(id, key, value) => editorStore.updateStyle(id, key, value)} onUpdateProp={(id, key, value) => editorStore.updateProp(id, key, value)} onDuplicate={(id) => editorStore.duplicateNode(id)} />
    ...
    {tool === "shape" && <div className="absolute bottom-4 left-1/2 z-10 -translate-x-1/2 rounded-full border border-border bg-white px-4 py-2 text-xs font-medium shadow-lg">Drag on the canvas to draw a rectangle · it locks after creation</div>}
  </div>;
}
...
function CanvasNode({ id, nodes, selected, onSelect }: { id: NodeId; nodes: Record<NodeId, EditorNode>; selected: NodeId | null; onSelect: (id: NodeId) => void }) {
  ...
  const isSelected = selected === id; const isShape = node.type === "shape";
  ...
  const beginDrag = (event: PointerEvent<HTMLDivElement>) => {
    if (!isSelected || node.locked || !isShape) return;
    event.stopPropagation(); const left = parseFloat(String(style.left ?? 0)) || 0; const top = parseFloat(String(style.top ?? 0)) || 0;
    drag.current = { x: event.clientX, y: event.clientY, left, top }; setInteracting(true); event.currentTarget.setPointerCapture(event.pointerId);
  };
  const moveDrag = (event: PointerEvent<HTMLDivElement>) => { if (!drag.current) return; const dx = event.clientX - drag.current.x; const dy = event.clientY - drag.current.y; editorStore.updateNode(node.id, { styles: { ...node.styles, left: `${Math.max(0, Math.round(drag.current.left + dx / 1))}px`, top: `${Math.max(0, Math.round(drag.current.top + dy / 1))}px`, position: "absolute" } }); };
  const endDrag = () => { drag.current = null; setInteracting(false); };
  const beginResize = (event: PointerEvent<HTMLDivElement>) => { if (!isSelected || node.locked || !isShape) return; event.stopPropagation(); resize.current = { x: event.clientX, y: event.clientY, width: parseFloat(String(style.width ?? 100)) || 100, height: parseFloat(String(style.height ?? 100)) || 100 }; setInteracting(true); event.currentTarget.setPointerCapture(event.pointerId); };
  const moveResize = (event: PointerEvent<HTMLDivElement>) => { if (!resize.current) return; const dx = event.clientX - resize.current.x; const dy = event.clientY - resize.current.y; editorStore.resizeNode(node.id, resize.current.width + dx, resize.current.height + dy); };
  const endResize = () => { resize.current = null; setInteracting(false); };
  return <div className={`${isShape ? "absolute" : "relative mb-4"} rounded-md border p-5 transition-shadow ${isSelected ? "border-primary ring-2 ring-primary/20" : "border-transparent hover:border-primary/30"} ${node.locked ? "select-none" : ""} ${interacting ? "cursor-grabbing" : ""}`} style={node.styles as CSSProperties} onPointerDown={beginDrag} onPointerMove={moveDrag} onPointerUp={endDrag} onClick={(event) => { event.stopPropagation(); onSelect(id); }}>
    {!isShape && <div className="mb-3 text-[10px] font-medium uppercase tracking-wider text-muted-foreground">{node.name}{node.locked ? " · Locked" : ""}</div>}
    {isShape ? null : node.type === "section" || node.type === "container" ? <div className="space-y-3 rounded border border-dashed border-border p-4">{node.childIds.length ? node.childIds.map((childId) => <CanvasNode key={childId} id={childId} nodes={nodes} selected={selected} onSelect={onSelect} />) : <span className="text-xs text-muted-foreground">Drop content here</span>}</div> : node.type === "heading" ? <h1 className="text-4xl font-bold text-foreground">{String(node.props.text ?? "Heading")}</h1> : node.type === "button" ? <button className="rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground">{String(node.props.text ?? "Button")}</button> : <p className="text-sm text-muted-foreground">{String(node.props.text ?? node.name)}</p>}
    {isShape && isSelected && !node.locked && <div onPointerDown={beginResize} onPointerMove={moveResize} onPointerUp={endResize} className="absolute -bottom-1.5 -right-1.5 size-3 cursor-se-resize rounded-sm border border-primary bg-white" title="Resize" />}
  </div>;
}
