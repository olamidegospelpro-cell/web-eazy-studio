...
export function HackathonStudio({ onClose, initialBusinessName = "Untitled project" }: { onClose: () => void; initialBusinessName?: string }) {
  ...
  const generate = async () => {
    ...
    try {
      ...
      setBlocks(r.blocks);
      setBusinessName(r.businessName || businessName);
  ...
  return (
    <div className="webeazy-studio fixed inset-0 z-[100] flex flex-col bg-[#090b10] text-white" onPointerDown={() => contextMenu && setContextMenu(null)}>
      {!immersive && <header className="webeazy-studio-header flex h-14 shrink-0 items-center justify-between border-b border-white/10 bg-[#10131a] px-2 sm:px-3">
        <div className="flex min-w-0 items-center gap-1.5">
          ...
          <img src="/webeazy-logo.svg" alt="WebEazy" className="size-8 rounded-lg object-contain p-1" />
        ...
        <div className="flex items-center gap-1.5">
          <button onClick={() => setAiOpen((v) => !v)} className={`hidden items-center gap-1.5 rounded-md border px-2.5 py-1.5 text-xs font-medium lg:flex ${aiOpen ? "border-[#22c55e]/35 bg-[#22c55e]/10 text-[#9af5b6]" : "border-white/10 bg-white/5 text-white/55"}`} title="Toggle AI assistant"><Bot className="size-3.5" />AI {aiOpen ? "On" : "Off"}</button>
          <div className="hidden items-center gap-1 rounded-lg border border-white/10 bg-black/20 p-1 md:flex">{(["desktop", "tablet", "mobile"] as Viewport[]).map((v) => <button key={v} title={v} onClick={() => setViewport(v)} className={`rounded-md p-1.5 ${viewport === v ? "bg-[#22c55e]/15 text-[#9af5b6]" : "text-white/35 hover:text-white"}`}>{v === "desktop" ? <Monitor className="size-3.5" /> : v === "tablet" ? <Tablet className="size-3.5" /> : <Smartphone className="size-3.5" />}</button>)}</div>
          ...
          <Button size="sm" onClick={exportHtml} disabled={!blocks.length} className="bg-[#22c55e] text-[#07110a] hover:bg-[#4ade80]"><Code2 className="mr-1.5 size-3.5" />Export</Button>
        </div>
      </header>}
      ...
      <div className="relative flex min-h-0 flex-1">
        {!preview && aiOpen && leftOpen && <aside className="hidden w-[255px] shrink-0 flex-col border-r border-white/10 bg-[#10131a] lg:flex">
          <div className="border-b border-white/10 p-3">
            <div className="mb-2 flex items-center justify-between"><div className="flex items-center gap-2 text-xs font-semibold text-white/65"><Bot className="size-4 text-[#a78bfa]" />Ask WebEazy</div><button title="Collapse AI" onClick={() => setLeftOpen(false)} className="rounded-md p-1 text-white/35 hover:bg-white/5 hover:text-white"><PanelLeftClose className="size-4" /></button></div>
            <Textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} className="min-h-28 resize-none border-white/10 bg-black/25 text-xs text-white placeholder:text-white/25" placeholder="Describe the business, style, pages and goals..." />
            <Button className="mt-2.5 w-full bg-[#8b5cf6] text-white shadow-[0_8px_24px_rgba(139,92,246,.16)] hover:bg-[#a78bfa]" onClick={() => void generate()} disabled={generating || !prompt.trim()}>{generating ? <RefreshCw className="mr-2 size-4 animate-spin" /> : <WandSparkles className="mr-2 size-4" />}{generating ? "Generating..." : blocks.length ? "Regenerate" : "Generate website"}</Button>
        ...
        <main className="relative min-w-0 flex-1 overflow-hidden bg-[#090b0c]">
          {preview ? <GeneratedSitePreview blocks={blocks} businessName={businessName} viewport={viewport} onBack={() => setPreview(false)} /> : <div className="flex min-h-full w-full flex-col">
            <StudioToolbar tool={tool} onTool={toolClick} onDuplicate={duplicateSelected} onDelete={deleteSelected} />
            <div className="min-h-0 flex-1 overflow-auto bg-[#0b0d0e] p-2 sm:p-3 lg:p-4"><div className="relative mx-auto min-h-[700px] overflow-auto rounded-[4px] border border-white/10 bg-[#151818] shadow-[0_20px_80px_rgba(0,0,0,.45)]" style={{ width: "100%", maxWidth: widths[viewport], height: "calc(100vh - 118px)" }}>
        ...
        {!preview && rightOpen && <aside className="hidden w-[250px] shrink-0 flex-col border-l border-white/10 bg-[#10131a] xl:flex">
          ...
          {selected ? <div className="min-h-0 flex-1 overflow-auto p-3">
            ...
            <div className="rounded-xl border border-[#8b5cf6]/20 bg-[#8b5cf6]/5 p-2.5"><div className="mb-2 flex items-center gap-2 text-xs font-semibold text-[#e4d9ff]"><Sparkles className="size-4" />AI refine</div><Textarea value={refine} onChange={(e) => setRefine(e.target.value)} className="min-h-16 resize-none border-white/10 bg-black/20 text-xs text-white" /><Button size="sm" className="mt-2 w-full bg-[#8b5cf6] text-white hover:bg-[#a78bfa]" onClick={() => void refineSelected()} disabled={refining || !aiOpen}>{refining ? <RefreshCw className="mr-2 size-3.5 animate-spin" /> : <Send className="mr-2 size-3.5" />}{aiOpen ? (refining ? "Applying..." : "Apply AI") : "AI is off"}</Button></div>
...
function StudioToolbar({ tool, onTool, onDuplicate, onDelete }: { tool: Tool; onTool: (tool: Tool) => void; onDuplicate: () => void; onDelete: () => void }) {
  ...
  return <div className="webeazy-canvas-toolbar flex h-10 shrink-0 items-center gap-0.5 overflow-x-auto border-b border-white/10 bg-[#10131a] px-2">
...
function ToolButton({ label, icon: Icon, onClick, active }: { label: string; icon: typeof MousePointer2; onClick: () => void; active?: boolean }) {
  return <button title={label} aria-label={label} onClick={onClick} className={`flex size-8 shrink-0 items-center justify-center rounded-md transition ${active ? "bg-[#8b5cf6]/20 text-[#d7c8ff] ring-1 ring-[#8b5cf6]/15" : "text-white/45 hover:bg-white/5 hover:text-white"}`}><Icon className="size-3.5" /></button>;
}
...
function DesignCanvas({ blocks, selectedId, onSelect, onUpdate, onAdd, onContextMenu, onGenerate, generating }: { blocks: DemoBlock[]; selectedId: string | null; onSelect: (id: string) => void; onUpdate: (id: string, patch: Partial<DemoBlock>) => void; onAdd: (block: DemoBlock) => void; onContextMenu: (event: MouseEvent<HTMLDivElement>, id: string) => void; onGenerate: () => void; generating: boolean }) {
  const addEmptyShape = () => onAdd({ id: `shape-${Math.random().toString(36).slice(2, 9)}`, type: "shape", name: "Shape", text: "", x: 180, y: 180, width: 300, height: 160, background: "#242728", color: "#f5f5e6", radius: 16, locked: false });
  return <div className="relative min-h-full w-full bg-[#151818]" onContextMenu={(e) => e.preventDefault()}>
    {blocks.length === 0 ? <div className="flex min-h-[700px] items-center justify-center p-8 text-center text-[#f5f5e6]"><div className="max-w-md"><div className="mx-auto mb-4 flex size-12 items-center justify-center rounded-xl bg-[#8b5cf6]/10 text-[#a78bfa] ring-1 ring-[#8b5cf6]/20"><Sparkles className="size-6" /></div><h2 className="text-xl font-bold">Start designing</h2><p className="mt-2 text-sm leading-relaxed text-white/45">Create freely with the toolbar, or turn on AI and generate a first draft.</p><div className="mt-4 flex flex-wrap justify-center gap-2"><Button variant="outline" onClick={addEmptyShape} className="border-white/10 bg-white/[0.03] text-white hover:bg-white/10"><Square className="mr-2 size-4" />Add shape</Button><Button className="bg-[#8b5cf6] text-white hover:bg-[#a78bfa]" onClick={onGenerate} disabled={generating}>{generating ? <RefreshCw className="mr-2 size-4 animate-spin" /> : <WandSparkles className="mr-2 size-4" />}Generate with AI</Button></div></div></div> : blocks.map((b, index) => <CanvasBlock key={b.id} block={b} layerIndex={index} selected={selectedId === b.id} onSelect={() => onSelect(b.id)} onUpdate={(p) => onUpdate(b.id, p)} onContextMenu={onContextMenu} />)}
  </div>;
}
...
function CanvasBlock({ block, layerIndex, selected, onSelect, onUpdate, onContextMenu }: { block: DemoBlock; layerIndex: number; selected: boolean; onSelect: () => void; onUpdate: (patch: Partial<DemoBlock>) => void; onContextMenu: (event: MouseEvent<HTMLDivElement>, id: string) => void }) {
  ...
  const cls = block.type === "heading" ? "px-2 text-5xl font-black leading-none tracking-tight" : block.type === "text" ? "px-2 text-lg leading-relaxed" : block.type === "button" ? "justify-center px-5 text-sm font-semibold shadow-lg" : block.type === "card" ? "border border-white/10 px-6 text-xl font-bold shadow-sm" : "";
  return <div style={style} onPointerDown={down} onPointerMove={move} onPointerUp={() => { drag.current = null; }} onContextMenu={(e) => onContextMenu(e, block.id)} className={`group flex touch-none select-none items-center overflow-hidden ${cls} ${selected ? "outline outline-2 outline-[#8b5cf6] outline-offset-2" : ""} ${block.locked ? "cursor-not-allowed" : "cursor-move"}`}>{block.type !== "shape" && <span contentEditable={!block.locked} suppressContentEditableWarning onPointerDown={(e) => e.stopPropagation()} onBlur={(e) => onUpdate({ text: e.currentTarget.textContent || "" })} className="w-full outline-none">{block.text}</span>}{block.locked && <Lock className="pointer-events-none absolute right-2 top-2 size-3 opacity-50" />}{selected && !block.locked && <div onPointerDown={resizeDown} onPointerMove={resizeMove} onPointerUp={() => { resize.current = null; }} className="absolute bottom-0 right-0 size-4 cursor-se-resize rounded-tl-md border-l border-t border-[#8b5cf6] bg-[#f5f5e6]" />}</div>;
}
...
function CanvasContextMenu({ menu, block, onAction, onDuplicate, onDelete, onClose }: { menu: { x: number; y: number; blockId: string }; block: DemoBlock | null; onAction: (id: string, direction: "front" | "back" | "forward" | "backward") => void; onDuplicate: () => void; onDelete: () => void; onClose: () => void }) {
  ...
  return <div onPointerDown={(e) => e.stopPropagation()} style={{ position: "fixed", left: Math.min(menu.x, window.innerWidth - 210), top: Math.min(menu.y, window.innerHeight - 260), zIndex: 200 }} className="w-52 overflow-hidden rounded-xl border border-white/10 bg-[#10131a] p-1.5 text-white shadow-2xl backdrop-blur-xl">
    ...
    {items.map((item) => <button key={item.label} onClick={item.action} className="w-full rounded-lg px-2.5 py-2 text-left text-xs text-white/75 hover:bg-[#8b5cf6]/15 hover:text-white">{item.label}</button>)}
...
function Field({ label, value, onChange, multiline }: { label: string; value: string; onChange: (value: string) => void; multiline?: boolean }) {
  return <label className="mb-2.5 block"><span className="mb-1 block text-[9px] font-semibold uppercase tracking-wide text-white/35">{label}</span>{multiline ? <Textarea value={value} onChange={(e) => onChange(e.target.value)} className="min-h-16 resize-none border-white/10 bg-black/20 text-xs text-white" /> : <input value={value} onChange={(e) => onChange(e.target.value)} className="h-8 w-full rounded-md border border-white/10 bg-black/20 px-2 text-[11px] text-white outline-none focus:border-[#22c55e]/50" />}</label>;
}
