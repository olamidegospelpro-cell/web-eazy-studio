/**
 * Native folder adapter — File System Access API today, Tauri tomorrow.
 *
 * The picked directory handle is persisted in IndexedDB so future saves reuse
 * the folder the user chose once ("remember the chosen location"). Browsers
 * require a user gesture to re-grant permission after a reload; that case is
 * surfaced as FsError("permission") so the UI can ask instead of silently
 * failing.
 */
...
async function loadHandle(location: ProjectLocation): Promise<DirHandle> {
  if (!location.handleId) {
    throw new FsError("not-found", "This project has no folder reference stored");
  }
  const handle = await idb.get<DirHandle>(STORE_HANDLES, location.handleId);
  if (!handle) {
    throw new FsError(
      "not-found",
      `WebEazy lost access to "${location.path}"`,
      "Use Save As to pick the project folder again.",
    );
  }
  ...
  if (state !== "granted") {
    ...
    if (granted !== "granted") {
      throw new FsError(
        "permission",
        `WebEazy needs permission to write to "${location.path}"`,
        "Click Grant access when your browser asks, or choose the folder again with Save As.",
      );
    }
  }
  return handle;
...
async function resolveDir(location: ProjectLocation, segments: string[], create: boolean) {
  ...
  for (const segment of segments) {
    try {
      ...
      throw new FsError(
        create ? "io" : "not-found",
        `Folder "${segment}" is missing in ${location.path}`,
        create ? "Check that you still have write access to the folder." : undefined,
      );
    }
  }
  return dir;
...
export const nativeAdapter: FsAdapter = {
  ...
  async pickDirectory(suggestedName) {
    ...
    if (!picker) {
      throw new FsError("unsupported", "This browser cannot open the system folder picker");
    }
    let handle: FileSystemDirectoryHandle;
    try {
      handle = await picker({ mode: "readwrite", id: "webeazy-projects", startIn: "documents" });
    } catch {
      throw new FsError("cancelled", "Folder selection was cancelled");
    }
    if (handle.name.startsWith(".")) {
      throw new FsError(
        "hidden-folder",
        `"${handle.name}" is a hidden folder`,
        "Pick a normal, visible folder such as Documents/WebEazy.",
      );
    }
  ...
  async ensureDir(location, relativePath) {
    await resolveDir(location, splitPath(relativePath), true);
  },

  async writeText(location, relativePath, contents) {
    const segments = splitPath(relativePath);
    const fileName = segments.pop();
    if (!fileName) throw new FsError("io", "A file name is required");
    const dir = await resolveDir(location, segments, true);
  ...
  async readText(location, relativePath) {
    ...
    try {
      ...
      throw new FsError("not-found", `Missing file: ${relativePath}`, "The project folder looks incomplete.");
    }
  },
  ...
  async remove(location, relativePath) {
    const segments = splitPath(relativePath);
    const last = segments.pop();
    if (!last) throw new FsError("io", "Refusing to delete the project root implicitly");
    const dir = await resolveDir(location, segments, false);
  ...
  async reveal(location) {
    throw new FsError(
      "unsupported",
      `Browsers cannot open "${location.path}" in your file manager`,
      "The desktop build of WebEazy will reveal the folder directly.",
    );
  },
};
