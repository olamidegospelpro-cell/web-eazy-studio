...
function wrap(error: unknown, fallback: string): ProjectError {
  ...
  return new ProjectError(
    fallback,
    "Check that the project folder is still available, then try again.",
  );
}
...
export const ProjectService = {
  ...
  async scaffold(location: ProjectLocation): Promise<void> {
    try {
      ...
      for (const folder of REQUIRED_FOLDERS) await FileSystemService.ensureDir(location, folder);
    } catch (error) {
    ...
    try {
      ...
      for (const ref of manifest.pages) {
        const parsed = pageSchema.safeParse(
          await FileSystemService.readJson<unknown>(location, ref.file),
        );
      ...
      const healed = ValidationService.heal(
        { ...manifest, settings: { ...manifest.settings, location } },
        theme,
        pages,
      );
  ...
  async readTheme(location: ProjectLocation): Promise<ProjectTheme> {
    if (await FileSystemService.exists(location, FILE_THEME)) {
      const parsed = themeSchema.safeParse(
        await FileSystemService.readJson<unknown>(location, FILE_THEME),
      );
  ...
  async duplicate(
    ...
    return ProjectService.writeAll(
      {
        ...bundle,
        manifest: {
          ...bundle.manifest,
